...
assert.deepEqual(provider.readiness_invocation.argv, ['node', '{{runtime_path}}/scripts/agent/homeboy-codex-provider-readiness.cjs']);
assert.equal(provider.readiness_invocation.env_allowlist.includes('AI_PROVIDER_OPENAI_CODEX_ACCESS_TOKEN'), true);
assert.equal(provider.readiness_invocation.env_allowlist.includes('HOMEBOY_CODEX_COMMAND_ARGS'), true);
