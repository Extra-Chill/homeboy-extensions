...
if (!data.timeline.find((event) => event.event === "http.status" && event.data.status === 503)) throw new Error("http status transition event missing");
...
if (statusHistory !== "502:2,503:1,200:1") throw new Error(`unexpected http status history: ${statusHistory}`);
if (httpSummary.data.repeated_status_count !== 1) throw new Error("http repeated status count missing");
if (httpSummary.data.last_non_ready_status !== 503) throw new Error("http last non-ready status missing");
if (!data.timeline.find((event) => event.event === "http.ready" && event.data.status === 200 && event.data.last_non_ready_status === 503)) throw new Error("http ready event missing summary");
