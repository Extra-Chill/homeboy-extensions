...
function normalizeWordPressAdminPageFuzzSurface(input, options = {}) {
	...
	return {
		...
		path: scenario.path,
		...(Array.isArray(scenario.interactions) ? { interactions: scenario.interactions } : {}),
		...(Array.isArray(scenario.forms) ? { forms: scenario.forms } : {}),
		...(Array.isArray(scenario.actions) ? { actions: scenario.actions } : {}),
		surfaceKind,
		status,
		skipReasons,
		unsafeReasons,
