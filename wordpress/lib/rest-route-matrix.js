...
const WORDPRESS_REST_ROUTE_INVENTORY_SCHEMA = 'homeboy/wordpress-rest-route-inventory/v1';
const WORDPRESS_REST_REQUEST_CASES_SCHEMA = 'homeboy/wordpress-rest-request-cases/v1';
...
function seededScore(value, seed) {
	...
	for (let index = 0; index < input.length; index += 1) {
		hash ^= input.charCodeAt(index); // eslint-disable-line no-bitwise
...
function buildRequestCase(matrixEntry, variant, values = {}, metadata = {}) {
	...
	if (Array.isArray(caseMetadata.expectedStatuses)) {
		expectedStatuses = caseMetadata.expectedStatuses;
	}
	return {
		...
		expectedStatuses,
		auth: caseMetadata.auth || { context: 'anonymous', role: null },
	};
}
...
function generateWordPressRestRequestCasesForEntry(matrixEntry, route = {}, options = {}) {
	...
	if (routeHasPathParams(matrixEntry.route) && pathArgs.length === 0) {
		return [];
	}
	...
	if (pathArgs.some(([name]) => pathParams[name] === undefined)) {
		return [];
	}
...
function unsupportedRestRequestCaseReason(entry = {}, route = {}) {
	const method = normalizeRestRouteMethod(entry.method);
	if (!SAFE_REST_REQUEST_METHODS.includes(method)) {
		return {
			type: 'unsafe_method',
			reason: `${method} is not generated by the safe REST request case generator.`,
		};
	}
	const args = routeArgsForMethod(route, method);
	const pathParams = new Set(routePathParamNames(entry.route));
	const argNames = new Set(args.map(([name]) => name));
	for (const name of pathParams) {
		if (!argNames.has(name)) {
			return {
				type: 'unsupported_parameterized_route',
				reason: `Path parameter ${name} has no matching REST arg metadata.`,
				arg: name,
			};
		}
	}
	for (const [name, arg] of args) {
		...
		if (safeValueForArg(name, arg) === undefined) {
			return {
				type: 'unsupported_parameterized_route',
				reason: `Path parameter ${name} has no generic safe value.`,
				arg: name,
			};
		}
	}
	if (routeHasPathParams(entry.route) && pathParams.size === 0) {
		return {
			type: 'unsupported_parameterized_route',
			reason: 'Route has a parameterized path without named WP REST parameters.',
		};
	}
	return null;
}

function restRequestCaseGap(entry, reason) {
	return {
		type: 'route_without_generated_request_case',
		id: entry.id,
		method: entry.method,
		path: entry.path,
		route: entry.route,
		namespace: entry.namespace,
		reason: reason?.reason || 'No generic safe request case was generated.',
		unsupportedType: reason?.type,
	};
}
...
function buildWordPressRestRequestCasesArtifact(input, options = {}) {
	const routes = extractRestRoutes(input);
	const matrix = normalizeWordPressRestRouteMatrix(input, {
		...options,
		methods: normalizeRequestMethodFilter(options),
	});
	const allRoutes = normalizeWordPressRestRouteMatrix(input, options);
	const maxCases = normalizeCaseLimit(options.maxCases);
	const cases = [];
	const skippedRoutes = [];
	const routeGaps = [];
	...
	for (const entry of allRoutes) {
		const route = routes[entry.route] || routes[normalizeRoutePath(entry.route)] || {};
		const unsupported = unsupportedRestRequestCaseReason(entry, route);
		const included = matrix.some((candidate) => candidate.id === entry.id);
		if (!included || unsupported) {
			const reason = unsupported || { type: 'filtered_method', reason: `${entry.method} was filtered out by request case options.` };
			skippedRoutes.push({
				id: entry.id,
				method: entry.method,
				path: entry.path,
			...
			if (unsupported && unsupported.type !== 'unsafe_method') {
				routeGaps.push(restRequestCaseGap(entry, unsupported));
			}
		}
	}
	...
	for (const entry of matrix) {
		...
		const generated = generateWordPressRestRequestCasesForEntry(entry, route, { ...options, maxCases: maxCases - cases.length });
		if (generated.length === 0) {
			routeGaps.push(restRequestCaseGap(entry));
		}
		cases.push(...generated);
	}

	return {
		schema: WORDPRESS_REST_REQUEST_CASES_SCHEMA,
		type: 'wordpress-rest-request-cases',
		generated_at: options.generatedAt || options.generated_at || new Date(0).toISOString(),
		filters: {
			methods: normalizeRequestMethodFilter(options),
			includeNamespaces: normalizeStringList(options.includeNamespaces ?? options.namespaces),
			excludeNamespaces: normalizeStringList(options.excludeNamespaces),
		},
		authContexts: options.authContexts || options.auth_contexts || [{ context: 'anonymous', role: null }],
		totals: {
		...
		cases,
		skippedRoutes: skippedRoutes.sort((a, b) => sortText(a.id, b.id)),
		routeGaps: routeGaps.sort((a, b) => sortText(a.id, b.id)),
	};
}

function extractRestRoutes(input) {
	if (Array.isArray(input?.routes)) {
		return routesArrayToRestRoutes(input.routes);
	}
...
function routesArrayToRestRoutes(entries = []) {
	const routes = {};
	for (const entry of entries) {
		if (!isPlainObject(entry) || !entry.route) {
			continue;
		}
		const route = normalizeRoutePath(entry.route);
		const method = normalizeRestRouteMethod(entry.method);
		const existing = routes[route] || { namespace: entry.namespace, methods: [], endpoints: [] };
		if (!existing.methods.includes(method)) {
			existing.methods.push(method);
		}
		existing.namespace = existing.namespace || entry.namespace;
		existing.endpoints.push({
			methods: [method],
			args: entry.args || {},
			schema: entry.responseSchema || entry.schema,
		});
		if (entry.responseSchema || entry.schema) {
			existing.schema = entry.responseSchema || entry.schema;
		}
...
function normalizeWordPressRestRouteInventory(input, options = {}) {
	const restRoutes = extractRestRoutes(input);
	const routes = normalizeWordPressRestRouteMatrix(input, options).map((entry) => {
		const route = restRoutes[entry.route] || restRoutes[normalizeRoutePath(entry.route)] || {};
		return {
			...entry,
			args: Object.fromEntries(routeArgsForMethod(route, entry.method)),
			responseSchema: routeSchema(route),
		};
	});
	return {
		schema: WORDPRESS_REST_ROUTE_INVENTORY_SCHEMA,
		type: 'wordpress-rest-route-inventory',
		generated_at: options.generatedAt || options.generated_at || new Date(0).toISOString(),
		totals: {
			routes: new Set(routes.map((entry) => entry.route)).size,
			entries: routes.length,
			namespaces: new Set(routes.map((entry) => entry.namespace).filter(Boolean)).size,
			methods: new Set(routes.map((entry) => entry.method).filter(Boolean)).size,
		},
		routes,
	};
}
