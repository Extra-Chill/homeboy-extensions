# WordPress Extension - Test Infrastructure Implementation Plan

## Overview

This plan describes adding WordPress test infrastructure to the WordPress extension, providing a centralized, zero-config testing and linting runtime for all WordPress projects.

### Key Principles

1. **Single Source of Truth**: WordPress core, test suite, and linting standards live in the extension, not in individual projects
2. **Zero Friction**: Projects only need test files; all infrastructure provided by extension
3. **Universal Standards**: All plugins/themes use same coding standards (PHPCS with WPCS)
4. **One-Time-Use Runtime**: WordPress runs ephemerally only during testing
5. **Consistent Platform**: All projects test and lint against same WordPress version and standards
6. **KISS Compliant**: Uses existing WordPress test suite and PHPCS patterns
7. **Platform-Agnostic**: Homeboy Core provides JSON only; extensions parse their own settings

### Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                     homeboy-extension-wordpress                   │
│  Runtime: Testing + Linting for WordPress ecosystem          │
├─────────────────────────────────────────────────────────────────┤
│                                                               │
│  Layer 1: Extension Files                                        │
│  ├─ wordpress.json       (manifest + runtime config)           │
│  ├─ composer.json       (dev deps: phpcs, phpstan)             │
│  ├─ package.json        (runtime dep: @wp-playground/cli)      │
│  ├─ phpcs.xml.dist     (universal coding standards)           │
│  └─ scripts/             (runtime scripts)                     │
│                                                               │
│  Layer 2: WordPress Playground Runtime                         │
│  ├─ PHP-WASM (embedded PHP, no host PHP needed for tests)     │
│  ├─ Embedded SQLite (no MySQL/SQLite shim needed)             │
│  ├─ Full WordPress (booted per test run, isolated)            │
│  └─ PHPUnit (runs inside Playground VFS)                      │
│                                                               │
│  Layer 3: Integration Points                                   │
│  ├─ build.sh           (existing - runs composer test)         │
│  ├─ homeboy test       (component & project testing)           │
│  ├─ composer lint      (PHPCS linting)                        │
│  └─ Extension runtime     (test-runner.sh → playground)       │
│                                                               │
└─────────────────────────────────────────────────────────────────┘
```

### Homeboy Core Contract

Homeboy Core provides these environment variables to extensions:

| Variable | Description | Example |
|----------|-------------|----------|
| `HOMEBOY_EXEC_CONTEXT_VERSION` | Exec context protocol version | `"1"` |
| `HOMEBOY_EXTENSION_ID` | Extension ID from manifest | `"wordpress"` |
| `HOMEBOY_SETTINGS_JSON` | **JSON string** of merged settings | `{"database_type":"sqlite"}` |
| `HOMEBOY_EXTENSION_PATH` | Extension filesystem path | `~/.config/homeboy/extensions/wordpress` |
| `HOMEBOY_PROJECT_PATH` | Project base_path (if project context) | `~/projects/my-plugin` |
| `HOMEBOY_PROJECT_ID` | Project ID (if set) | `"my-plugin"` |
| `HOMEBOY_COMPONENT_ID` | Component ID (if set) | `"staging"` |
| `HOMEBOY_COMPONENT_PATH` | Component filesystem path (if set) | `~/components/my-plugin` |

**Important:** Extensions must parse `HOMEBOY_SETTINGS_JSON` themselves using tools like jq. Homeboy Core does NOT expand individual `HOMEBOY_SETTINGS_*` environment variables.

---

## 1. Extension Directory Structure

```
homeboy-extensions/wordpress/
├── wordpress.json                          # Extension manifest
├── composer.json                           # Dev dependencies (phpcs, phpstan)
├── package.json                            # Runtime deps (@wp-playground/cli, eslint)
├── phpcs.xml.dist                          # Universal PHPCS config
├── phpunit.xml.dist                        # PHPUnit config (for extension self-tests)
├── vendor/                                 # Generated by composer install
│   ├── squizlabs/php_codesniffer/          # PHPCS
│   ├── wp-coding-standards/wpcs/           # WPCS
│   ├── phpstan/                            # Static analysis
│   └── bin/
│       ├── phpcs                           # Linting tool
│       └── phpstan                         # Static analysis tool
├── node_modules/                           # Generated by npm install
│   └── @wp-playground/cli/                 # Playground CLI binary
├── scripts/
│   ├── build/
│   │   └── setup.sh                        # Initial setup (npm + composer install)
│   ├── test/
│   │   ├── test-runner.sh                  # Thin router → playground
│   │   ├── test-runner-playground.sh       # Playground backend (the only backend)
│   │   ├── playground-runner.php           # PHP template run inside Playground
│   │   └── playground-dropin-smoke.sh      # db.php drop-in regression test
│   ├── lint/
│   │   └── lint-runner.sh                  # PHPCS linting
│   └── ...
├── tests/
│   └── fixtures/
│       └── dropin-coexistence/             # db.php drop-in test fixture
└── docs/
    └── PLAYGROUND_DROPIN.md                # Drop-in coexistence documentation
```

---

## 2. Extension Manifest (wordpress.json)

```json
{
  "name": "WordPress",
  "version": "1.0.0",

  "runtime": {
    "setup_command": "bash {{extensionPath}}/scripts/setup.sh",
    "ready_check": "test -f {{extensionPath}}/vendor/wp-phpunit/wp-phpunit/tests/phpunit/includes/bootstrap.php",
    "run_command": "bash {{extensionPath}}/scripts/test-runner.sh"
  },

  "testing": {
    "supported_databases": ["sqlite", "mysql"],
    "default_database": "sqlite"
  },

  "commands": ["wp", "wp-test"],

  "settings": {
    "database_type": {
      "type": "select",
      "options": ["sqlite", "mysql"],
      "default": "sqlite",
      "description": "Database backend for tests (sqlite = in-memory, fastest)"
    },
    "mysql_host": {
      "type": "string",
      "default": "localhost",
      "description": "MySQL host (when using mysql database type)"
    },
    "mysql_database": {
      "type": "string",
      "default": "wordpress_test",
      "description": "MySQL database name (when using mysql database type)"
    },
    "mysql_user": {
      "type": "string",
      "default": "root",
      "description": "MySQL user (when using mysql database type)"
    },
    "mysql_password": {
      "type": "string",
      "default": "",
      "description": "MySQL password (when using mysql database type)"
    }
  }
}
```

---

## 3. Composer Configuration (composer.json)

```json
{
  "name": "homeboy/extension-wordpress",
  "description": "WordPress extension with embedded test infrastructure and linting",
  "type": "homeboy-extension",
  "license": "GPL-2.0-or-later",

  "require": {
    "php": ">=7.4",
    "wp-phpunit/wp-phpunit": "^6.8"
  },

  "require-dev": {
    "phpunit/phpunit": "^9.0",
    "squizlabs/php_codesniffer": "^3.10",
    "wp-coding-standards/wpcs": "^3.1",
    "dealerdirect/phpcodesniffer-composer-installer": "^1.0"
  },

  "config": {
    "allow-plugins": {
      "dealerdirect/phpcodesniffer-composer-installer": true
    }
  },

  "scripts": {
    "setup": [
      "@composer install"
    ],
    "test": "phpunit --configuration={{extensionPath}}/phpunit.xml.dist",
    "lint": "phpcs"
  }
}
```

**Key additions:**
- PHPCS dependencies for linting
- `composer lint` script for universal linting

---

## 4. Universal PHPCS Configuration (phpcs.xml.dist)

```xml
<?xml version="1.0"?>
<ruleset name="homeboy-wp-standard">
    <description>Universal PHPCS standard for all WordPress projects using Homeboy</description>

    <arg name="colors" value="true"/>
    <arg value="ps"/>

    <rule ref="WordPress">
        <exclude name="WordPress.WP.I18n.MissingTranslatorsComment"/>
        <exclude name="WordPress.WP.DeprecatedParameters"/>
    </rule>

    <exclude-pattern>*/vendor/*</exclude-pattern>
    <exclude-pattern>*/node_extensions/*</exclude-pattern>
    <exclude-pattern>*/build/*</exclude-pattern>
    <exclude-pattern>*/dist/*</exclude-pattern>

    <arg name="extensions" value="php"/>
</ruleset>
```

**Design decisions:**
- Uses WordPress Coding Standards as base
- Includes reasonable exclusions for development directories
- Universal - no plugin/theme customization needed
- Provided by extension, not required in projects

---

## 5. Runtime Scripts

### scripts/setup.sh
```bash
#!/bin/bash
set -euo pipefail

EXTENSION_PATH="${HOMEBOY_EXTENSION_PATH}"

echo "Setting up WordPress test infrastructure..."

# Install dependencies
cd "$EXTENSION_PATH"
composer install --quiet --no-interaction

echo "WordPress test infrastructure installed successfully"
echo "WP_TESTS_DIR: $EXTENSION_PATH/vendor/wp-phpunit/wp-phpunit/tests/phpunit"
echo "ABSPATH: $EXTENSION_PATH/vendor/wp-phpunit/wp-phpunit/wordpress"
echo "PHPCS available: composer lint"
```

### scripts/test-runner.sh

Supports both component-level and project-level testing:

**Component-level testing:**
- Lints single component with PHPCS
- Runs PHPUnit if component has tests/
- Fails fast if linting fails

**Project-level testing:**
- Lints ALL components in project
- Runs PHPUnit on components with tests/
- Reports per-component results

### scripts/generate-config.sh

Handles SQLite and MySQL database configuration for WordPress testing environment.

---

## 6. Bootstrap Files

### tests/bootstrap.php
Loads WordPress core and the plugin/theme being tested.

### phpunit.xml.dist
PHPUnit configuration pointing to extension bootstrap.

---

## 7. SQLite Database Driver

### sqlitedb/db.php
Lightweight SQLite driver for in-memory database testing.

---

## 8. Project Requirements

### Minimal Project Structure
```
my-plugin/
├── my-plugin.php                    # Plugin main file
├── inc/                             # Plugin code
└── tests/                           # ONLY test files (no WordPress!)
    ├── test-sample.php
    └── (no bootstrap.php needed)
```

### What Projects DON'T Need:
- ❌ WordPress installation
- ❌ wp-config.php
- ❌ Database setup
- ❌ PHPCS configuration (phpcs.xml)
- ❌ PHPCS packages
- ❌ PHPUnit configuration
- ❌ Test bootstrap

### What Projects DO Need:
- ✅ Test files in `tests/` directory
- ✅ That's it!

---

## 9. Build Integration

The existing `build.sh` runs tests and linting:

```bash
# Build.sh runs these automatically:
composer lint        # PHPCS linting (extension provides)
composer test        # PHPUnit tests (extension provides)
```

---

## 10. Running Tests and Linting

### Component-Level Testing

```bash
# Test single component (lint + tests if exist)
homeboy test blocks-everywhere

# Debug mode
HOMEBOY_DEBUG=1 homeboy test blocks-everywhere
```

**Behavior:**
1. Lints component with PHPCS
2. Fails if linting errors
3. Runs PHPUnit if `tests/` exists
4. Skips PHPUnit if no tests

### Project-Level Testing

```bash
# Test entire project (all components)
homeboy test extra-chill

# Debug mode
HOMEBOY_DEBUG=1 homeboy test extra-chill
```

**Behavior:**
1. Lints ALL components in project
2. Fails fast if any component has linting errors
3. Runs PHPUnit on components with `tests/`
4. Reports per-component results

### Extension Runtime

```bash
# Run WordPress extension with custom settings
homeboy extension run wordpress --component blocks-everywhere \
  --setting database_type=sqlite
```

### Composer Commands

```bash
# Lint current directory (uses extension PHPCS)
composer lint

# Test current directory (uses extension PHPUnit)
composer test
```

---

## 11. Version Management

### Extension Version = WordPress Version

```json
{
  "name": "WordPress",
  "version": "6.8.1",
  "wordpress_core_version": "6.8.1",
  "wp_phpunit_version": "6.8.1"
}
```

### Update Process

1. WordPress releases new version (e.g., 6.8.2)
2. Update composer.json to require `"wp-phpunit/wp-phpunit": "^6.8.2"`
3. Update version in wordpress.json to "6.8.2"
4. Users run: `homeboy extension update wordpress`
5. Extension runs `composer update` → new WordPress installed
6. All projects now test and lint against new WordPress version

---

## 12. Implementation Status

### ✅ Phase 1: Extension Infrastructure (COMPLETE)

| Task | File | Status |
|------|------|--------|
| Create composer.json with wp-phpunit | composer.json | ✅ Complete |
| Create composer.json with PHPCS | composer.json | ✅ Complete |
| Add `composer lint` script | composer.json | ✅ Complete |
| Create scripts/setup.sh | scripts/setup.sh | ✅ Complete |
| Create scripts/test-runner.sh | scripts/test-runner.sh | ✅ Complete |
| Create scripts/generate-config.sh | scripts/generate-config.sh | ✅ Complete |
| Create tests/bootstrap.php | tests/bootstrap.php | ✅ Complete |
| Create phpunit.xml.dist | phpunit.xml.dist | ✅ Complete |
| Create phpcs.xml.dist | phpcs.xml.dist | ✅ Complete |
| Create SQLite driver | sqlitedb/db.php | ✅ Complete |

### ✅ Phase 2: Extension Manifest (COMPLETE)

| Task | File | Status |
|------|------|--------|
| Add runtime section | wordpress.json | ✅ Complete |
| Add testing section | wordpress.json | ✅ Complete |
| Add settings array | wordpress.json | ✅ Complete |
| Add wp-test command | wordpress.json | ✅ Complete |
| Extension ready check | wordpress.json | ✅ Complete |

### ✅ Phase 3: PHPCS Linting (COMPLETE)

| Task | Status |
|------|--------|
| Add PHPCS dependencies | ✅ Complete |
| Add `composer lint` script | ✅ Complete |
| Create universal phpcs.xml.dist | ✅ Complete |
| Update test-runner.sh for PHPCS | ✅ Complete |
| Project-level linting support | ✅ Complete |
| Component-level linting support | ✅ Complete |

### ✅ Phase 4: Test Execution (COMPLETE)

| Task | Status |
|------|--------|
| Component-level testing | ✅ Complete |
| Project-level testing | ✅ Complete |
| PHPUnit only when tests exist | ✅ Complete |
| Per-component reporting | ✅ Complete |

### ✅ Phase 5: Playground Backend (COMPLETE — Phases 1 & 2 of #214)

Playground backend that runs PHPUnit inside WordPress Playground (PHP-WASM + embedded SQLite).

| Task | File | Status |
|------|------|--------|
| Add `@wp-playground/cli` to package.json | package.json | ✅ Complete |
| Create test-runner-playground.sh | scripts/test/test-runner-playground.sh | ✅ Complete |
| In-process WP install (replace `system()` call) | playground-runner.php | ✅ Complete |
| Structured stage logging + diagnostics | playground-runner.php, test-runner-playground.sh | ✅ Complete |
| Parse extension phpunit.xml.dist + recursive discovery | playground-runner.php | ✅ Complete |
| db.php drop-in coexistence + smoke test | tests/fixtures/dropin-coexistence/, playground-dropin-smoke.sh | ✅ Complete |

**Diagnostics contract:**

The template writes a structured log (`.pg-test-result.txt`) with these line patterns:

| Line | Meaning |
|------|---------|
| `STAGE_BEGIN:<stage>` | Entered a bootstrap phase |
| `STAGE_OK:<stage>` | Phase completed cleanly |
| `STAGE_FAIL:<stage>:<msg>` | Throwable caught during phase |
| `STAGE_FATAL:<stage>:<msg>` | Uncatchable fatal (shutdown handler) |
| `NOTICE:<msg>` | PHP warning/notice (not swallowed) |
| `PLUGIN_DETECTED <file>` / `THEME_DETECTED` | Component load success |
| `NO_TEST_FILES` | Discovery found nothing |
| `ALL TESTS PASSED` / `SOME TESTS FAILED` | PHPUnit outcome |

Stages (in execution order): `boot` → `install` → `load_fixtures` → `load_deps`
→ `load_component` → `discover_tests` → `load_tests` → `run_tests`.

**Architecture (current — after Phase 6):**

```
┌─────────────────────────────────────────────────────────────────┐
│                  test-runner.sh (thin router)                    │
│  Resolves context, exports env vars, execs Playground runner    │
└─────────────────────────────────────────────────────────────────┘
                              │
                  test-runner-playground.sh
                              │
┌─────────────────────────────────────────────────────────────────┐
│  1. Generate PHP wrapper (env vars + bootstrap + PHPUnit)       │
│  2. Build mount list (plugin + deps + extension)                │
│  3. Run via: wp-playground-cli php --mount ... -- /runner.php   │
│  4. Capture results via host-mounted .pg-test-result.txt        │
└─────────────────────────────────────────────────────────────────┘
```

### ✅ Phase 6: Host Backend Retirement (COMPLETE — Phase 3 of #214)

Retired the legacy host-PHP test backend entirely. Single code path: Playground only.

**Removed:**
- `sqlitedb/db.php` — 765-line `FakeMySQL` + `SQLite_DB extends wpdb` shim
- `scripts/test/generate-config.sh` — generated wp-tests-config.php for host backend
- `tests/bootstrap.php` — host-only wp-phpunit bootstrap
- `wp-phpunit/wp-phpunit` dependency from composer.json
- MySQL/SQLite auto-detection, WordPress download caching, wp-phpunit config proxy
- `test_backend`, `database_type`, and `mysql_*` settings from wordpress.json

**Changed:**
- `test-runner.sh` — rewritten as thin router that always execs `test-runner-playground.sh`
- `setup.sh` — removed wp-phpunit references, npm install is now the primary step
- `wordpress.json` — `ready_check` now verifies Playground CLI, not wp-phpunit
- `phpunit.xml.dist` — removed wp-phpunit env var references

**Known Gaps (remaining):**

1. **WP version pinning.** The `--wp` flag is pinned (currently 6.9.x). Mismatch
   causes missing class errors.

2. **db.php drop-in coexistence depends on Playground upstream.** See
   `docs/PLAYGROUND_DROPIN.md` for the full mechanism.

3. **Partial phpunit.xml.dist consumption.** The runner reads `<testsuite><directory>`
   and `<exclude>` entries only. Other elements are intentionally not honored.

**References:**
- Issue: Extra-Chill/homeboy-extensions#214
- Playground CLI: https://www.npmjs.com/package/@wp-playground/cli

---

## 13. Benefits Summary

### Testing
✅ **Zero WordPress infrastructure in projects** - No wp-config.php, no database setup, no WordPress installation
✅ **Universal testing** - One WordPress installation serves all projects
✅ **Zero-config** - SQLite in-memory database, no setup needed
✅ **Fast** - No database server, in-memory database
✅ **Consistent** - All projects test against same WordPress version
✅ **Version controlled** - Extension updates update WordPress centrally
✅ **Build integration** - Automatic testing during builds

### Linting
✅ **Universal standards** - All projects use same coding standards (WPCS)
✅ **Zero-config** - PHPCS provided by extension, no setup needed
✅ **Project-level linting** - Lint all components in project at once
✅ **Fail-fast** - Catch issues before running tests
✅ **No PHPCS in projects** - Extension provides all PHPCS packages
✅ **Universal config** - Same rules for everyone

### Platform
✅ **Platform offering** - Homeboy provides complete development infrastructure
✅ **Platform-agnostic** - Homeboy Core provides JSON only; extensions parse settings themselves
✅ **Single source of truth** - WordPress, PHPUnit, PHPCS all from extension

---

## 14. FAQ

### Testing Questions

**Q: Do I need to install WordPress in each project?**
A: No. The WordPress extension provides WordPress centrally. Projects only need test files.

**Q: How do I specify which WordPress version to test against?**
A: The WordPress extension version determines the WordPress test version. Update the extension to test against new WordPress versions.

**Q: Can I use MySQL instead of SQLite?**
A: Tests run inside WordPress Playground which uses embedded SQLite. No MySQL configuration needed.

**Q: Do I need a wp-config.php in my project?**
A: No. Playground provides the entire WordPress environment.

**Q: How does database cleanup work?**
A: Each Playground instance is ephemeral — the database is created fresh for every test run and destroyed when it completes.

**Q: Can I test against multiple WordPress versions?**
A: Currently not supported (per KISS principle). Extension version = WordPress version.

**Q: What if my plugin requires other plugins to be active?**
A: Add those plugins to test bootstrap via `tests_add_filter('muplugins_loaded', ...)`

**Q: Does this work for themes too?**
A: Yes. The test bootstrap will look for style.css with "Theme Name:" header and load it similarly.

### Linting Questions

**Q: Do I need PHPCS in my project?**
A: No. The WordPress extension provides PHPCS and all standards. Just run `composer lint`.

**Q: Do I need phpcs.xml in my project?**
A: No. The extension provides a universal phpcs.xml.dist with WordPress standards.

**Q: Can I customize PHPCS rules for my project?**
A: Currently no (per KISS principle). Universal standards ensure consistency across all projects.

**Q: What coding standards are used?**
A: WordPress Coding Standards (WPCS) - the official standard for WordPress development.

**Q: How do I lint all components in a project?**
A: Run `homeboy test <project-id>` - it lints all components then runs tests on those with tests.

**Q: What if my project doesn't have tests?**
A: It will still be linted. PHPUnit is skipped if no `tests/` directory exists.

**Q: Can I run just linting without tests?**
A: Run `composer lint` in any WordPress project - it will use the extension's PHPCS.

### Architecture Questions

**Q: Why use jq to parse JSON instead of individual env vars?**
A: Homeboy Core must remain platform-agnostic. Providing individual HOMEBOY_SETTINGS_* vars assumes extension-specific patterns. Extensions are responsible for parsing their own settings.

**Q: Why single source of truth?**
A: Eliminates duplication, ensures consistency, simplifies maintenance, and provides a centralized upgrade path for WordPress, PHPUnit, and PHPCS.

**Q: Can I use my own PHPUnit or PHPCS?**
A: No (per KISS principle). The extension provides everything. If you need custom infrastructure, you're on your own.

**Q: What about CI/CD integration?**
A: Simply run `composer lint` and `composer test` in your CI pipeline. The extension provides all infrastructure.

---

## 15. Architectural Notes

### Homeboy Core Responsibilities
- Provide `HOMEBOY_SETTINGS_JSON` as a single JSON string
- Provide generic context vars: HOMEBOY_EXTENSION_PATH, HOMEBOY_PROJECT_PATH, HOMEBOY_COMPONENT_ID, HOMEBOY_COMPONENT_PATH
- Remain completely agnostic to extension implementation details
- Execute extension runtime scripts

### Extension Responsibilities
- Parse `HOMEBOY_SETTINGS_JSON` using tools like jq
- Extract settings needed for extension-specific behavior
- Implement all platform-specific logic within extension boundaries
- Handle database configuration, WordPress specifics, PHPCS configuration
- Provide universal PHPCS standards and configuration
- Support both component-level and project-level operations

### Anti-Patterns to Avoid
- Homeboy Core expanding individual settings as `HOMEBOY_SETTINGS_*`
- Extensions assuming platform-specific env vars exist
- Tight coupling between Homeboy Core and any specific extension
- Projects managing their own PHPCS configuration or packages
- Duplicating WordPress, PHPUnit, or PHPCS infrastructure in projects

### Separation of Concerns
This separation ensures:
- Any extension can use the same Homeboy Core
- Homeboy Core remains simple and maintainable
- Extensions can evolve independently
- Platform-specific concerns stay within extensions
- Projects focus on code, not infrastructure
- Universal standards and tooling across all projects

---

## 16. Future Enhancements

### Potential Future Features
- Support for testing against multiple WordPress versions
- PHPCS rule customization per project (if demand exists)
- Static analysis integration (PHPStan, Psalm)
- Coverage reporting
- Performance testing integration

### Principles for Future Work
- Maintain KISS principle
- Keep universal infrastructure in extension
- Avoid project-level duplication
- Prioritize consistency over flexibility
