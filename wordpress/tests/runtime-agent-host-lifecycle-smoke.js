...

{
  ...
  fs.writeFileSync(path.join(repo, 'docs', 'architecture.md'), '# Architecture\n');
  const { configPath, resultsPath } = makeRunFiles(tmp, {
    writable_paths: ['README.md', 'docs/**', 'plugins/**/README.md', 'plugins/**/AGENTS.md'],
    declared_side_effect_paths: ['plugins/**/AGENTS.md'],
    verification_commands: [{
      command: 'mkdir -p plugins/amp && printf "generated by build\\n" > plugins/amp/AGENTS.md',
      description: 'Verifier creates generated package output',
    }],
  ...
  const scenario = output.scenarios[0];
  assert.equal(scenario.metrics.writable_paths_satisfied, 1);
  assert.equal(scenario.metrics.side_effect_policy_satisfied, 1);
  assert.equal(scenario.metrics.pr_opened, 1);
  assert.deepEqual(scenario.metadata.runner_workspace_capture.agent_files, ['docs/architecture.md']);
  ...
  assert.match(publishedFiles, /plugins\/amp\/AGENTS\.md/);
}

{
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'homeboy-host-lifecycle-undeclared-side-effect.'));
  const repo = makeRepo(tmp);
  const gh = makeGhFixture(tmp);
  fs.writeFileSync(path.join(repo, 'generated.txt'), 'hello from agent\n');
  const { configPath, resultsPath } = makeRunFiles(tmp, {
    writable_paths: ['generated.txt', 'build/**'],
    verification_commands: [{
      command: 'mkdir -p build && printf "generated by verifier\\n" > build/output.txt',
      description: 'Verifier creates undeclared output',
    }],
  });
  ...
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Verification side-effect files outside declared policy: build\/output\.txt/);
  const output = readJson(resultsPath);
  const scenario = output.scenarios[0];
  assert.equal(output.status, 'failed');
  assert.equal(scenario.metrics.side_effect_policy_satisfied, 0);
  assert.equal(scenario.metrics.pr_opened, 0);
  assert.deepEqual(scenario.metadata.runner_side_effect_policy.rejected_files, ['build/output.txt']);
  assert.equal(fs.existsSync(gh.log), false);
}
