...
assert.equal(typeof wordpress.wpCodebox.buildWpCodeboxFuzzPlanRecipeLegacyRunAlias, 'function');
...
assert.equal(typeof wordpress.buildWpCodeboxFuzzPlanRecipeLegacyRunAlias, 'function');
...
assert.equal(wordpress.wpCodebox.WORDPRESS_CODEBOX_FUZZ_RUN_CONSUMER_SCHEMA, wordpress.wpCodebox.WORDPRESS_CODEBOX_FUZZ_SUITE_CONSUMER_SCHEMA);
...
assert.equal(wordpress.wpCodeboxFuzzRunTaskRequest({ taskId: 'public-run-compat' }).executor.config.runtime_task.input.schema, 'wp-codebox/fuzz-suite/v1');
