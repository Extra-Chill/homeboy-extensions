...
const attribution = normalizeWordPressVisualAttribution({
	...
	visualExplanation: {
		...
		changes: [ { path: 'main > p', tag: 'p', changes: { boundingBox: { source: { x: 1 }, candidate: { x: 3 }, delta: { x: 2, y: 0, width: 0, height: 0 }, severity: 'warning' }, styles: { 'font-size': { source: '16px', candidate: '18px', category: 'typography', severity: 'warning', hint: 'Text changed.' }, 'background-color': { source: 'red', candidate: 'blue', category: 'paint', severity: 'warning', hint: 'Paint changed.' } } } } ],
...
const changeOnly = normalizeWordPressVisualAttribution({
	visualExplanation: {
		schema: 'wp-codebox/visual-explanation/v1',
		changes: [ { path: 'main > button', tag: 'button', changes: { boundingBox: { source: { x: 1 }, candidate: { x: 4 }, delta: { x: 3, y: 0, width: 0, height: 0 }, severity: 'warning' }, styles: { color: { source: 'red', candidate: 'blue', category: 'paint', severity: 'warning', hint: 'Color changed.' } } } } ],
	},
