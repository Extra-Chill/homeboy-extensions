...
cp "${WORDPRESS_ROOT}/scripts/lib/validation-dependencies.sh" \
    "${WORDPRESS_ROOT}/scripts/lib/wp-scripts-toolchain.sh" \
    "${FIXTURE_ROOT}/extension-sources/wordpress/scripts/lib/"
...
# Resolve the actual core helper through Homeboy when this sealed fixture has no
# adjacent checkout. An explicit binding remains authoritative and is validated
# by the shared resolver below.
# shellcheck source=../../scripts/lib/runtime-helper-resolver.sh
source "${REPOSITORY_ROOT}/scripts/lib/runtime-helper-resolver.sh"
if [ -z "${HOMEBOY_RUNTIME_SETTINGS_HELPER:-}" ] && command -v homeboy >/dev/null 2>&1; then
    export HOMEBOY_RUNTIME_SETTINGS_HELPER="$(homeboy runtime helper path settings.sh --plain)"
fi
SETTINGS_HELPER="$(homeboy_runtime_helper "$REPOSITORY_ROOT" HOMEBOY_RUNTIME_SETTINGS_HELPER settings.sh)"
...
HOMEBOY_SHARED_LIB_DIR="${FIXTURE_ROOT}/extensions/scripts/lib" \
